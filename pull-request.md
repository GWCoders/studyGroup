...
4. Add the original Study Group repo using this command:
  ```
  git remote add upstream https://github.com/GWCoders/studyGroup.git
  git fetch upstream
  ```
...
8. Push up to your GitHub repo:
  ...
  ```
9. Submit a Pull Request *from* your GitHub account *into* the GW Coders.
Make sure the **base fork** is set to `GWCoders/studyGroup` (and not `mozillascience/studyGroup`).
